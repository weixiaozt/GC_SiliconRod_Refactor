...
## 三·B、不改现场源码的部署（方案 B / site 钩子）

和「方式 A」（拷 .pyd + 改 `main_camera.py` 两行）相比，方案 B **一行 app 源码都不改**，靠
Python 启动钩子生效；现场以后升级 app 版本也不用再打补丁。已用一键脚本在全新 venv 实测通过。

**原理**：Python 启动会自动加载 site-packages 里的 `.pth`，丢一个一行的 `sirod_lock.pth`
（`import sirod_license_hook`）进去触发钩子；钩子**只在启动 `main_camera.py` / `main.py` 时**
校验授权，`pip` 等其它调用一律放行。要装的 3 个文件：

| 文件 | 来源 | 版本 |
|---|---|---|
| `license_guard.<abi>.pyd` | 你编的 | 盐城 cp310 / 宜宾 cp311（**认 python 版本，别拿错**） |
| `sirod_license_hook.py` | `tools/` | 两个现场通用 |
| `sirod_lock.pth` | `tools/` | 两个现场通用 |

**一键脚本**（★推荐★，自动按现场 python 版本挑对的 .pyd、拷 3 文件、装 cryptography、自检）：
双击 `scripts\deploy\install_hook.bat`，或 `python scripts\deploy\install_hook.py`。
卸载：`install_hook.bat --uninstall`。

> ⚠️ 一键脚本要用**实际启动 main_camera.py 的那个 python** 来跑（launcher 用 PATH 上的
> python；脚本会打印它用的是哪个 python，核对一下）。它会按该 python 的版本自动挑 cp310/cp311。

> **方式 A vs B**：A 把闸门写进 app（直接 `python main_camera.py` 也拦得住，最稳），但每次
> 升级 app 要重加那两行；B 完全不碰源码、随 app 升级免维护，代价是能被 `python -S` 绕过、
> site-packages 那 3 个文件也是 weak link。防操作工/私自复制够用。**二选一，用 B 就别再改 main_camera.py**。

> **续期 / 改永久 / 换机**：方案 B 与方式 A **完全一样**，见下面 §四（都是对 `license.dat`
> 用 `renew` 操作，跟锁怎么装无关）。下面两段只写各自的「加锁」。

---

### ▌▌ 盐城 · 方案B（Python 3.10 → cp310）▌▌

前提：盐城的 `license_guard.cp310-win_amd64.pyd` 已 build 好（在仓库 `build\` 下，或 U 盘带到现场）。

`[盐城电脑]`
```bat
:: 1. 一键装锁（自动认出 3.10 → 挑 cp310、拷 3 文件、装 cryptography、自检）
scripts\deploy\install_hook.bat
:: 2. 取机器码，复制打印的 blob 发开发
python tools\get_machine_id.py
```
`[你的电脑]`（有 private_key.pem）
```bat
python tools\license_gen.py issue --machine <盐城blob> --expires 2027-06-30 --customer "盐城" --key <私钥路径>
::   → license.dat 发回盐城（自己也留档）
```
`[盐城电脑]`
```bat
:: 3. license.dat 放项目根目录
:: 4. ★必测★：正常启动能起；把 license.dat 改个名再启动 → 应"授权校验失败"起不来
```
- 续期 / 改永久：见 §四（`renew`，覆盖 license.dat）。
- 拆锁：`scripts\deploy\install_hook.bat --uninstall`。

---

### ▌▌ 宜宾 · 方案B（Python 3.11 → cp311）▌▌

和盐城一模一样，只差三处：**cp311**、备注写「宜宾」、存宜宾自己那份 license.dat（和盐城分开）。

`[宜宾电脑]`
```bat
:: 1. 一键装锁（自动认出 3.11 → 挑 cp311）
scripts\deploy\install_hook.bat
:: 2. 取机器码发开发
python tools\get_machine_id.py
```
`[你的电脑]`
```bat
python tools\license_gen.py issue --machine <宜宾blob> --expires 2027-06-30 --customer "宜宾" --key <私钥路径>
```
`[宜宾电脑]`
```bat
:: 3. license.dat 放项目根；4. ★必测★ 改名 license.dat 后启动应起不来
```
- 续期 / 改永久：见 §四。拆锁：`scripts\deploy\install_hook.bat --uninstall`。
